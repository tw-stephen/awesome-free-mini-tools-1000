...
export default function PlantWateringReminder() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('plant-watering-821')
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('plant-watering-821', JSON.stringify(plants))
  }, [plants])
  ...
  const addPlant = () => {
    ...
    setNewPlant({ name: '', location: 'Living Room', wateringInterval: '7', notes: '' })
  ...
  return (
    <div className="space-y-4">
      <div className="card p-4">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div className="p-3 bg-green-50 rounded">
            ...
            <div className="text-xs text-slate-500">{t('tools.plantWateringReminder.totalPlants')}</div>
          </div>
          <div className="p-3 bg-red-50 rounded">
            <div className="text-2xl font-bold text-red-600">{stats.needsWater}</div>
            <div className="text-xs text-slate-500">{t('tools.plantWateringReminder.needsWater')}</div>
          </div>
          <div className="p-3 bg-yellow-50 rounded">
            <div className="text-2xl font-bold text-yellow-600">{stats.upcoming}</div>
            <div className="text-xs text-slate-500">{t('tools.plantWateringReminder.upcoming')}</div>
          </div>
        </div>
      </div>
      ...
      {!showAddPlant ? (
        ...
        >
          + {t('tools.plantWateringReminder.addPlant')}
        </button>
      ) : (
        <div className="card p-4">
          ...
          <div className="space-y-3">
            <input
              ...
              placeholder={t('tools.plantWateringReminder.plantName')}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm text-slate-500">{t('tools.plantWateringReminder.days')}</span>
              </div>
            </div>
            <input
              type="text"
              ...
              placeholder={t('tools.plantWateringReminder.notes')}
              className="w-full px-3 py-2 border border-slate-300 rounded text-sm"
            />
      ...
      {sortedPlants.length === 0 ? (
        <div className="card p-8 text-center text-slate-500">
          {t('tools.plantWateringReminder.noPlants')}
        </div>
      ) : (
        <div className="card p-4">
          <h3 className="text-sm font-medium text-slate-700 mb-3">
            {t('tools.plantWateringReminder.yourPlants')}
          </h3>
          <div className="space-y-3">
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="text-sm text-slate-500">
                        {plant.location} - {t('tools.plantWateringReminder.every')} {plant.wateringInterval} {t('tools.plantWateringReminder.days')}
                      </div>
                      {plant.notes && (
                  ...
                  <div className="flex items-center justify-between">
                    <span className={`text-sm font-medium ${
                      ...
                      {needsWater
                        ? t('tools.plantWateringReminder.waterNow')
                        : `${daysUntil} ${t('tools.plantWateringReminder.daysUntil')}`}
                    </span>
                    <button
                      onClick={() => waterPlant(plant.id)}
                      className="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600"
                    >
                      {t('tools.plantWateringReminder.water')}
                    </button>
                  </div>
                </div>
              )
